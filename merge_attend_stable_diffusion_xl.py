...
class StableDiffusionXLPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        views = get_views(height, width, window_size=128, stride=stride)
        ...
        count = torch.zeros_like(latents, requires_grad=False, device=self.device, dtype=self.dtype)
        value = torch.zeros_like(latents, requires_grad=False, device=self.device, dtype=self.dtype)
        ...
        prompt_embeds = prompt_embeds[:, None].repeat(1, len(views), 1, 1).reshape(b * len(views), l, d)
        b, d = add_text_embeds.size()
        add_text_embeds = add_text_embeds[:, None].repeat(1, len(views), 1).reshape(b * len(views), d)
        b, d = add_time_ids.size()
        add_time_ids = add_time_ids[:, None].repeat(1, len(views), 1).reshape(b * len(views), d)
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_view_dn = self.scheduler.step(noise_pred, t, torch.cat(batched_latent_views), **extra_step_kwargs, return_dict=False)[0]
